...
public class Dictionary {
    ...
    public Dictionary(Context context,String tableName){
        ...
        dbHelper=new DB(context, tableName);
        //这里要用到前面的DataBaseHelper类，在Dict的构造方法中实例化该类，
        //并且调用下面两个方法获得dbR和dbW,用于完成对数据库的增删改查操作。
        //这里把dbR dbW作为成员变量目的是避免反复实例化dbR  dbW造成数据库指针泄露。
...
//                "interpret text, sentorig text, senttrans text)");
    ...
    public WordValue getWordFromInternet(String searchedWord){
        ...
        String str=null;
        try{
            //tempUrl金山api查单词，返回一个xml，里面有单词结果
            String tempUrl= DbConnectApi.iCiBaURL1+tempWord+ DbConnectApi.iCiBaURL2;
            in= DbConnectApi.getInputStreamByUrl(tempUrl);
            if(in!=null){
                //  new FileUtils().saveInputStreamToFile(in, "", "gfdgf.txt");
                ApiXmlParser xmlParser=new ApiXmlParser();
